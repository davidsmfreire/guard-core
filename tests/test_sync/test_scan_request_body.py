...
def test_body_scanned_by_default() -> None:
...
def test_body_not_read_when_scanning_disabled() -> None:
    request = _FakeRequest(b"<script>alert(1)</script>")

    result = detect_penetration_attempt(
        cast(SyncGuardRequest, request), SecurityConfig(scan_request_body=False)
    )
...
def test_disabling_body_scan_keeps_query_scanning() -> None:
