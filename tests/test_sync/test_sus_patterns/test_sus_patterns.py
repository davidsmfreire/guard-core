...
def test_invalid_pattern_handling() -> None:
    ...
    sus_patterns_handler.add_pattern(bad_pattern, custom=True)
    assert bad_pattern not in sus_patterns_handler.custom_patterns


def test_unsupported_pattern_is_skipped() -> None:
    # Lookaround is unsupported by RE2; the pattern is skipped, not added.
    lookaround = r"(?=secret)token"
    sus_patterns_handler.add_pattern(lookaround, custom=True)
