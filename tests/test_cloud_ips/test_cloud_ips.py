...
async def test_cloud_ip_redis_caching(security_config_redis: SecurityConfig) -> None:
    with (
        patch("guard_core.handlers.cloud_handler.fetch_aws_ip_ranges") as mock_aws,
        patch("guard_core.handlers.cloud_handler.fetch_gcp_ip_ranges"),
        patch("guard_core.handlers.cloud_handler.fetch_azure_ip_ranges"),
    ):
        mock_aws.return_value = {ipaddress.IPv4Network("192.168.0.0/24")}
...
async def test_cloud_ip_redis_cache_hit(security_config_redis: SecurityConfig) -> None:
    ...
    with patch("guard_core.handlers.cloud_handler.fetch_aws_ip_ranges") as mock_aws:
        assert cloud_handler.is_cloud_ip("192.168.0.1", {"AWS"})
